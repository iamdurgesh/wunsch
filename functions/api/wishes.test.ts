...
function request(body: unknown = { answers, note: 'Ein Tag am Meer.', version: questionnaireVersion }, headers: Record<string, string> = {}) {
  return new Request('https://birthday.example/api/wishes', { method: 'POST', headers: { Origin: 'https://birthday.example', 'Content-Type': 'application/json', 'X-Invite-Token': token, ...headers }, body: JSON.stringify(body) });
}

describe('private wish submission', () => {
  it('stores a readable result and updates the same row on retry or edit', async () => {
    expect((await handleWishes(request(), { DB, INVITE_TOKEN: token })).status).toBe(200);
    expect((await handleWishes(request({ answers, note: '  Ein Ausflug!  ', version: questionnaireVersion }), { DB, INVITE_TOKEN: token })).status).toBe(200);
  ...
  it('rejects unauthorized, cross-origin and public read requests', async () => {
    expect((await handleWishes(request(undefined, { 'X-Invite-Token': 'b'.repeat(64) }), { DB, INVITE_TOKEN: token })).status).toBe(401);
    expect((await handleWishes(request(undefined, { Origin: 'https://another.example' }), { DB, INVITE_TOKEN: token })).status).toBe(403);
    expect((await handleWishes(new Request('https://birthday.example/api/wishes'), { DB, INVITE_TOKEN: token })).status).toBe(405);
    expect(sqlite.prepare('SELECT * FROM wishes').all()).toHaveLength(0);
  });

  it.each([
    { answers: {}, note: '', version: questionnaireVersion },
    { answers: { ...answers, fitness: 'unknown' }, note: '', version: questionnaireVersion },
    { answers, note: 'x'.repeat(501), version: questionnaireVersion },
    { answers, note: '', version: 'old-questionnaire' },
  ])('rejects incomplete, unknown, oversized or stale answers', async (body) => {
    expect((await handleWishes(request(body), { DB, INVITE_TOKEN: token })).status).toBe(400);
    expect(sqlite.prepare('SELECT * FROM wishes').all()).toHaveLength(0);
  });

  it('bounds streamed body size and fails closed if storage is not configured', async () => {
    expect((await handleWishes(request({ data: 'x'.repeat(17000) }), { DB, INVITE_TOKEN: token })).status).toBe(413);
