  ...
  test(`questionnaire fits ${viewport.width} × ${viewport.height}`, async ({ page }, testInfo) => {
    ...
    for (let index = 1; index < questions.length; index++) {
      if (questions[index].id === 'season') {
        ...
        await noOverflow();
        if (viewport.width === 390) await page.screenshot({ path: testInfo.outputPath('mobile-season-reaction.png'), fullPage: true, animations: 'disabled' });
        await page.getByRole('button', { name: 'Hinweis schließen' }).click();
        await expect(page.getByText('Gleichgesinnte!', { exact: true })).toHaveCount(0);
      }
    ...
    if (viewport.width === 390) {
      ...
      await page.waitForFunction(() => {
        const canvas = document.querySelector<HTMLCanvasElement>('.celebration-confetti');
        const context = canvas?.getContext('2d');
        if (!canvas || !context) return false;
        const pixels = context.getImageData(0, Math.floor(canvas.height * .45), canvas.width, 60).data;
        let painted = 0;
        for (let index = 3; index < pixels.length; index += 4) if (pixels[index] > 80) painted++;
        return painted > 100;
      });
      await page.screenshot({ path: testInfo.outputPath('mobile-confetti.png') });
    }
    ...
    if (viewport.width === 390) {
      ...
      await page.getByRole('button', { name: `${questions.at(-1)!.category}: Antwort ändern` }).click();
