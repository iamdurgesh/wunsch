...
import { SelectionReaction } from "./SelectionReaction";

function GiftArt({ small = false }: { small?: boolean }) {
  return (
    <div
      className={`gift-scene ${small ? "gift-scene-small" : ""}`}
      aria-hidden="true"
    >
...
function App() {
  ...
  function restart() {
    if (
      !window.confirm(
        "Möchten Sie neu anfangen? Ihr Entwurf in diesem Tab wird gelöscht. Bereits gesendete Wünsche bleiben beim Empfänger gespeichert.",
      )
    )
      return;
    ...
    >
      ...
      <header className="site-header">
        <a
          ...
          aria-label="Für Frau Inge – zur Begrüßung"
        >
          <span className="brand-icon">
            <Gift size={22} strokeWidth={1.5} />
          </span>
          <span>
            Für Frau Inge<span className="brand-dot">.</span>
          </span>
        </a>
        <span className="header-note">
          <span /> Mit Liebe ausgedacht
        </span>
      </header>

      <main id="main-content">
        <div className="journey" aria-label="Fortschritt">
          <span className={step < 2 ? "journey-active" : "journey-complete"}>
            <span className="journey-number">
              {step >= 2 ? <Check size={12} /> : "01"}
            </span>{" "}
            Ein kleines Hallo
          </span>
          ...
          <span
            className={
              isQuestion ? "journey-active" : isDone ? "journey-complete" : ""
            }
          >
            <span className="journey-number">
              {isDone ? <Check size={12} /> : "02"}
            </span>{" "}
            Ihre Wünsche
          </span>
          <span className="journey-line" />
          <span className={isDone ? "journey-active" : ""}>
            <span className="journey-number">03</span> Die Vorfreude
          </span>
        </div>
        ...
        <div className="card-stack">
          ...
          >
                ...
                <>
                  <section className="letter-panel">
                    <div className="eyebrow">
                      <span className="tiny-line" /> EINE KLEINE
                      GEBURTSTAGSMISSION
                    </div>
                    ...
                    <p className="letter-lead">
                      ...
                      <br />
                      Meine Geschenkidee?{" "}
                      <span className="underlined">Noch in Arbeit.</span>
                    </p>
                    <p className="body-copy">
                      Wie schon im letzten Jahr bin ich auch dieses Jahr ein
                      bisschen ideenlos. Aber dieses Mal habe ich einen Plan:
                      Wir finden gemeinsam heraus, was Ihnen Freude macht.
                    </p>
                    <p className="letter-invitation">
                      Ein paar Fragen. Ein bisschen Spaß.
                      <br />
                      Und am Ende vielleicht die perfekte Idee.
                    </p>
                    <button
                      className="primary-button"
                      onClick={() => setStep(1)}
                    >
                      Na dann, los geht’s <ArrowRight size={18} />
                    </button>
                    <div className="button-caption">
                      <LockKeyhole size={12} /> Nur für Sie. Und Ihre
                      Geburtstagswünsche.
                    </div>
                  </section>
                  <aside className="art-panel">
                    <div className="round-stamp">
                      EIN KLEINES<span>Nur für Sie</span> BISSCHEN VORFREUDE
                    </div>
                    <GiftArt />
                    <div className="handwritten">
                      Was schenkt man jemandem,
                      <br />
                      der „eigentlich nichts“ braucht?
                    </div>
              ...
              {step === 1 && (
                <>
                  <section className="letter-panel ready-panel">
                    <div className="eyebrow">
                      <Sparkles size={14} /> BEVOR WIR LOSLEGEN
                    </div>
                    <h1 ref={titleRef} tabIndex={-1}>
                      Bereit für ein
                      <br />
                      bisschen <em>Neugier?</em>
                    </h1>
                    <p className="body-copy">
                      {questions.length} kleine Fragen helfen mir auf die
                      Sprünge. Es gibt keine falschen Antworten – nur gute
                      Hinweise.
                    </p>
                    <div className="ready-facts">
                      <span>
                        <Check size={17} /> Ganz entspannt, in Ihrem Tempo
                      </span>
                      <span>
                        <Check size={17} /> Antworten jederzeit ändern
                      </span>
                      <span>
                        <Check size={17} /> Zum Schluss Platz für Ihren eigenen
                        Wunsch
                      </span>
                    </div>
                    <button
                      ...
                      onClick={() => setStep(2)}
                    >
                      Ich bin bereit <ArrowRight size={18} />
                    </button>
                    <p className="button-caption">
                      Kaffee in der Hand? Dann kann’s losgehen.
                    </p>
                  </section>
                  <aside className="art-panel meme-panel">
                    <div className="meme-label">ICH, BEIM GESCHENKEFINDEN</div>
                    <div className="meme-emoji" aria-hidden="true">
                      🧐
                    </div>
                    <h2>
                      Dieses Jahr
                    ...
                    <span className="meme-sticker">100 % gut gemeint</span>
                  </aside>
                </>
              )}
              ...
              {isQuestion && question && (
                <>
                  <section className="letter-panel question-panel">
                    <div className="eyebrow">
                      FRAGE {String(questionIndex + 1).padStart(2, "0")}{" "}
                      <span className="eyebrow-divider">/</span>{" "}
                      {question.category.toLocaleUpperCase("de-DE")}
                    </div>
                    <h1 className="question-title" ref={titleRef} tabIndex={-1}>
                      {question.title}
                    </h1>
                    <p className="body-copy question-description">
                  ...
                  <aside className="art-panel meme-panel">
                    {question.meme ? (
                      <figure className="custom-meme">
                        <img src={question.meme.src} alt={question.meme.alt} />
                        <figcaption>{question.meme.caption}</figcaption>
                      </figure>
                    ) : (
                      <>
                        <div className="meme-label">
                          EIN KLEINER GEDANKE AM RANDE
                        </div>
                        <div className="meme-emoji" aria-hidden="true">
                          {question.aside.emoji}
                        </div>
                        <h2>{question.aside.headline}</h2>
                        <p>{question.aside.caption}</p>
                        <span className="meme-sticker">
                          Man wird ja wohl wünschen dürfen.
                        </span>
                      </>
                    )}
                  </aside>
              ...
              {isDone && (
                <>
                  <section className="letter-panel result-panel">
                    <div className="eyebrow">
                      <CheckCheck size={16} /> MISSION: EIN GUTES STÜCK WEITER
                    </div>
                    <h1 ref={titleRef} tabIndex={-1}>
                      Weniger Rätsel.
                      <br />
                    ...
                    <p className="body-copy">
                      Vielen Dank, Frau Inge! Mit diesen kleinen Hinweisen wird
                      aus meiner Ideenlosigkeit langsam ein Plan.
                    </p>
                    <div className="answer-summary">
                      {getAnswerRows(answers).map(
                        ({ question: answeredQuestion, option }, index) => (
                          <div
                          ...
                          >
                            ...
                            <div>
                              <small>{answeredQuestion.category}</small>
                              <strong>{option?.label ?? "Noch offen"}</strong>
                            </div>
                            <button
                              className="text-button"
                              onClick={() => setStep(index + 2)}
                              aria-label={`${answeredQuestion.category}: Antwort ändern`}
                            >
                              Ändern
                            </button>
                          </div>
                    ...
                    <label className="note-label" htmlFor="wish-note">
                      Oder gibt es schon einen Herzenswunsch?{" "}
                      <span>(optional)</span>
                    </label>
                    <textarea
                      id="wish-note"
                      ...
                      placeholder="Was ich mir eigentlich wünsche …"
                      rows={2}
                    />
                    <SubmitWishes
                      onInteraction={recordInteraction}
                  ...
                  <aside className="art-panel result-art">
                    <span className="result-confetti" aria-hidden="true">
                      ✦ · ✧ · ✦
                    </span>
                    <GiftArt small />
                    <div className="handwritten">
                      Vorfreude ist schließlich
                      <br />
                      auch schon ein Geschenk.
                    </div>
                    <span className="art-footnote">
                      FORTSETZUNG FOLGT. MIT GESCHENKPAPIER.
                    </span>
                  </aside>
                </>
              )}
            ...
            <div className="card-bottom">
              <span>
                {step === 0
                  ? "DER ANFANG VON EINER GUTEN IDEE"
                  : isDone
                    ? "DAS WAR’S SCHON. FAST WIE ZAUBEREI."
                    : "KLEINE FRAGEN, GROSSE GESCHENKIDEEN"}
              </span>
              <span>
                {String(step).padStart(2, "0")}{" "}
                <span className="counter-divider">/</span>{" "}
                {String(lastStep).padStart(2, "0")}
              </span>
            </div>
            <div
              className="progress-track"
              role="progressbar"
              aria-label="Geburtstagsmission"
        ...
        {!draftSaved && (
          <p className="draft-warning" role="status">
            Ihr Browser kann den Entwurf gerade nicht merken. Bitte lassen Sie
            diesen Tab bis zum Senden geöffnet.
          </p>
        )}
        <div className="below-card">
          ...
          <span>
            <Heart size={13} /> Kein gewöhnlicher Fragebogen. Für keine
            gewöhnliche Frau.
          </span>
          {isDone ? (
            ...
            <span className="below-spacer" />
          )}
        </div>
      </main>

      <footer className="site-footer">
        <span>EINE KLEINE IDEE. VON HERZEN.</span>
        <details className="privacy-details">
          <summary>
            <LockKeyhole size={12} /> Was passiert mit meinen Antworten?
          </summary>
          <p>
        ...
        <span>
          Mit einem kleinen bisschen Magie <Sparkles size={13} />
        </span>
      </footer>
    </div>
  );
