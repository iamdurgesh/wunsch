...
describe('birthday questionnaire', () => {
  it('requires a choice, retains edited answers, and confirms before restarting', async () => {
    ...
    render(<App />);
    await user.click(screen.getByRole('button', { name: 'Na dann, los geht’s' }));
    await user.click(screen.getByRole('button', { name: 'Ich bin bereit' }));
    expect((screen.getByRole('button', { name: 'Nächste Frage' }) as HTMLButtonElement).disabled).toBe(true);
    ...
    expect((screen.getByRole('radio', { name: /Vielleicht/ }) as HTMLInputElement).checked).toBe(true);
    await user.click(screen.getByRole('radio', { name: /Lieber ganz ohne Zahlen/ }));
    await user.click(screen.getByRole('button', { name: 'Nächste Frage' }));

    for (let index = 1; index < questions.length; index++) {
      await user.click(screen.getAllByRole('radio')[0]);
      await user.click(screen.getByRole('button', { name: index === questions.length - 1 ? 'Zu meinen Wünschen' : 'Nächste Frage' }));
    }
    expect(screen.getByRole('progressbar').getAttribute('aria-valuenow')).toBe('100');
    expect(screen.getByText('Lieber ganz ohne Zahlen.')).toBeTruthy();
    await user.type(screen.getByRole('textbox', { name: /Herzenswunsch/ }), 'Ein Tag am Meer.');
