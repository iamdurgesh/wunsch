...
async function readBody(request: Request): Promise<unknown> {
  ...
  try {
    while (true) {
      ...
      if (size > MAX_BODY_BYTES) { await reader.cancel(); throw new RangeError('Too large'); }
...
export async function handleWishes(request: Request, env: Env): Promise<Response> {
  if (request.method !== 'POST') return new Response(null, { status: 405, headers: { ...headers, Allow: 'POST' } });
  if (request.headers.get('Origin') !== new URL(request.url).origin) return json({ error: 'Forbidden' }, 403);
  if (!env.INVITE_TOKEN || !/^[a-f0-9]{64}$/.test(env.INVITE_TOKEN) || !env.DB) return json({ error: 'Not configured' }, 503);
  const token = request.headers.get('X-Invite-Token') ?? '';
  if (!/^[a-f0-9]{64}$/.test(token)) return json({ error: 'Unauthorized' }, 401);
  const [expected, received] = await Promise.all([digest(env.INVITE_TOKEN), digest(token)]);
  let difference = 0;
  ...
  let body: unknown;
  try { body = await readBody(request); } catch (error) { return json({ error: 'Invalid body' }, error instanceof RangeError ? 413 : 400); }
  if (!body || typeof body !== 'object' || Array.isArray(body)) return json({ error: 'Invalid answers' }, 400);
  const payload = body as Record<string, unknown>;
  if (payload.version !== questionnaireVersion || !isValidAnswers(payload.answers, true) || typeof payload.note !== 'string' || payload.note.length > 500) return json({ error: 'Invalid answers' }, 400);

  // One row per invitation: retries and deliberate edits replace the same result.
  const invitationId = Array.from(expected, (byte) => byte.toString(16).padStart(2, '0')).join('');
  try {
    await env.DB.prepare(`INSERT INTO wishes (invitation_id, answers_json, note, summary, updated_at)
      VALUES (?, ?, ?, ?, ?)
      ...
      .bind(invitationId, JSON.stringify(payload.answers), payload.note.trim(), buildSummary(payload.answers, payload.note), new Date().toISOString()).run();
